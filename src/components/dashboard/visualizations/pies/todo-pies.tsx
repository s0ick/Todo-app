...
export const TodoPies: FC<PiesProps> = memo(({data, size, lang}) => {
  ...
  const outerScale = useMemo(() => scaleOrdinal({
  ...
  const innerScale = useMemo(() => scaleOrdinal({
  ...
  const weeklyData = useMemo(() => data[PiesTypes.INNER][0].value || data[PiesTypes.INNER][1].value, [data]);

  const {
    radius, centerY, centerX, donutThickness
  } = useMemo(() => {
    ...
    return {
      radius: r, centerY: cy, centerX: cx, donutThickness: donut
    };
  }, [size]);
  ...
  return (
    <TodoPiesWrapper>
      <svg width={size} height={size} ref={containerRef}>
        <GradientPinkBlue id="visx-gradient-green-l" fromOpacity={0.7} />
        <GradientTealBlue id="visx-gradient-green-d" fromOpacity={0.7} />

        <GradientPinkBlue id="visx-gradient-pink" fromOpacity={0.5} toOpacity={0.7} />
        <GradientTealBlue id="visx-gradient-teal" fromOpacity={0.5} toOpacity={0.7} />
